...
from lifegit.providers.chatgpt import ChatGPTProvider
from lifegit.providers.claude import ClaudeProvider
from lifegit.qdw_export import export_qdw_jsonl
from lifegit.enrichment import export_batches, apply_results
from lifegit.pastme import snapshot
from lifegit.api import query
...
class TestLifeGit(unittest.TestCase):
    ...
    def _import_both(self):
        for provider,path in [(ChatGPTProvider(),FIX/'chatgpt_conversations.json'),(ClaudeProvider(),FIX/'claude_conversations.json')]:
            convs,sha,member=provider.parse(path)
            self.db.import_conversations(convs,artifact_sha=sha,provider=provider.name,source_path=str(path),member_name=member)
    ...
    def test_provenance_and_private_default(self):
        self._import_both(); run_extraction(self.db)
    ...
    def test_qdw_export_retains_private_space(self):
        self._import_both(); run_extraction(self.db)
        out=self.root/'qdw.jsonl'; result=export_qdw_jsonl(self.db,out,space_id='life:test-user')
    ...
    def test_enrichment_contract(self):
        ...
        enriched.write_text(json.dumps({
            ...
            'objects':[{'object_type':'IDEA','canonical_text':'Build a provenance-backed personal timeline','confidence':0.91,'work_relevance':0.5,'attributes':{'source':'mock'}}]
        })+'\n')
